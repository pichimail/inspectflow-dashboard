...
import {PanelLeftClose,PanelLeftOpen,Search,MessageSquareText,Sun,Moon,RefreshCw,Mic,Volume2,X,Send,ChevronDown} from 'lucide-react';
 ...
 return <div className={`app-shell ${collapsed?'sidebar-collapsed':''}`}>
  ...
  <aside className={`sidebar ${drawer?'mobile-open':''}`} aria-label="Primary navigation">
   ...
   <nav className="sidebar-nav">
    {groups.map(section=><div key={section} className="nav-group"><div className="nav-section">{section}</div>{ROUTES.filter(r=>r.section===section).map(r=>{const Icon=r.icon;return <div key={r.href}><Link href={r.href} className={`nav-link ${isActive(path,r.href)?'active':''}`} data-collapse-tip={r.label}><Icon size={18}/><span>{r.label}</span>{r.badge&&<em>{r.badge}</em>}</Link>{!collapsed&&r.children&&isActive(path,r.href)&&<div className="subnav">{r.children.map(([href,label])=><Link className={path===href?'active':''} key={href} href={href}>{label}</Link>)}</div>}</div>})}</div>)}
   </nav>
  ...
  <section className="workspace">
   <header className="topbar">
    <TipButton label={collapsed?'Expand navigation':'Collapse navigation'} side="bottom" onClick={()=>{if(innerWidth<900)setDrawer(true);else setCollapsed(v=>!v)}}>{collapsed?<PanelLeftOpen size={18}/>:<PanelLeftClose size={18}/>}</TipButton>
    <div className="breadcrumb"><span>InspectFlow</span><b>/</b><strong>{current}</strong></div>
    <button type="button" className="command-trigger" onClick={()=>setCommand(true)}><Search size={17}/><span>Search or command</span><kbd>⌘K</kbd></button>
    <div className="top-actions">
     <TipButton label="Refresh data" side="bottom" onClick={()=>router.refresh()}><RefreshCw size={17}/></TipButton>
     <TipButton label="Open InspectFlow Agent" side="bottom" onClick={()=>setAgent(true)}><MessageSquareText size={17}/></TipButton>
     <TipButton label={`Switch to ${theme==='dark'?'light':'dark'} mode`} side="bottom" onClick={()=>setTheme(theme==='dark'?'light':'dark')}>{theme==='dark'?<Sun size={17}/>:<Moon size={17}/>}</TipButton>
